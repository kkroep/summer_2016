...
\draw (-1,2.5) node[anchor=east]{IN[i]} to (Vg.E);
...
\draw (Reset.G) to (2.5,4.5) node[anchor=south]{\Rst};
\draw (Reset.C) -| (3.5,0.5) node[dot]{};
\draw (5,0.5) node[dot]{} to (CM_L1.C);
\draw (CM_L1.G) to (4,4.5) node[anchor=south]{\Res}; 
\draw (CM_L1.E) to (CM_L2.E) to (7,-2.5) node[anchor=north]{\gnd};
\draw (CM_H1.B) |- (CM_H2.C) to (7,4.5) node[anchor=south]{\VDD};
...
\draw (7,1.5)node[dot]{} to (10,1.5) node[anchor=west]{\OUT};
\draw (9,1)node[dot]{} to (10,1) node[anchor=west]{\VBO};
